...
public class Day12 extends Day {
    ...
    public void move() {
        for (int i = 0; i < instructions.length; i++) {
            ...
            if (instructions[i] == 'S') move(0, -values[i]);
            if (instructions[i] == 'E') move(values[i], 0);
            if (instructions[i] == 'W') move(-values[i], 0);
            if (instructions[i] == 'F') move(values[i]*xDirections[currentDir], values[i]*yDirections[currentDir]);
